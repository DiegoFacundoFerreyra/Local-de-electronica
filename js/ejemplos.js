...
function mostrar_autos() {
  if (existen_autos) {
    let resp = prompt(
      "La info se mostrara ordenada por precio.\n Desea verla en forma Ascendente (A) o Desendente (D)"
    ).toUpperCase();
    if (resp == "A") {
...
//ESTO ES UN CARRITO YA MAS AVANZADO, CON COMPRAS QUE SE REPITEN POR PRODUCTO

function mostrarMenu() {
  let salir = false;
  let carrito = {}; // ahora usamos un objeto para contar cantidades
  ...
  while (!salir) {
    ...
    if (producto) {
      // Acumular en el carrito
      if (carrito[producto]) {
        carrito[producto]++; // ya existe → sumo cantidad
      } else {
        carrito[producto] = 1; // primera vez → cantidad = 1
      }
